...
public partial class StorageStructure : FactoryStructure, IFactoryItemProvider, IFactoryItemReceiver
{
    ...
    protected override void BuildVisuals()
    {
        ...
        {
            CreateBox("Base", new Vector3(CellSize * 0.92f, 0.16f, CellSize * 0.92f), new Color("0F172A"), new Vector3(0.0f, 0.08f, 0.0f));
            CreateInteriorModuleShell(this, "StorageCabinet", new Vector3(CellSize * 0.78f, 0.90f, CellSize * 0.76f), new Color("334155"), new Color("94A3B8"), new Vector3(0.0f, 0.68f, 0.0f));
            CreateBox("OutputStripe", new Vector3(CellSize * 0.14f, 0.08f, CellSize * 0.38f), new Color("FBBF24"), new Vector3(CellSize * 0.30f, 1.08f, 0.0f));

            for (var index = 0; index < 4; index++)
            {
                var indicator = CreateBox(
                    $"Fill_{index}",
                    new Vector3(CellSize * 0.10f, 0.10f, CellSize * 0.42f),
                    new Color("67E8F9"),
                    new Vector3(-CellSize * 0.18f + index * CellSize * 0.12f, 1.08f, 0.0f));
                indicator.Visible = false;
                _fillIndicators.Add(indicator);
            }

            _statusBeacon = CreateBox("Beacon", new Vector3(CellSize * 0.14f, 0.14f, CellSize * 0.14f), new Color("E2E8F0"), new Vector3(0.0f, 1.28f, 0.0f));
            return;
        }
